...
use sentinel_core::{Capability, CapabilityEvent, CapabilityIssued, CapabilityRevoked, CapabilityConsumed};
use uuid::Uuid;
...
impl CapabilityState {
	pub fn reduce<I: IntoIterator<Item = CapabilityEvent>>(events: I, valid_actors: &HashSet<Uuid>) -> Result<Self, String> {
			...
			match event {
				CapabilityEvent::CapabilityIssued(CapabilityIssued { capability, .. }) => {
					if !valid_actors.contains(&capability.actor_id) {
						return Err(format!("Cannot issue capability for unknown actor: {}", capability.actor_id));
					}
					if state.active.contains_key(&capability.capability_id) {
						return Err(format!("Duplicate capability_id: {}", capability.capability_id));
					}
					state.by_actor.entry(capability.actor_id).or_default().insert(capability.capability_id);
				...
				CapabilityEvent::CapabilityRevoked(CapabilityRevoked { capability_id, .. }) => {
					if !state.active.contains_key(&capability_id) {
						return Err(format!("Cannot revoke unknown capability: {}", capability_id));
					}
					state.revoked.insert(capability_id);
					state.active.remove(&capability_id);
				}
				CapabilityEvent::CapabilityConsumed(CapabilityConsumed { capability_id, .. }) => {
					if !state.active.contains_key(&capability_id) {
						return Err(format!("Cannot consume unknown or revoked capability: {}", capability_id));
					}
