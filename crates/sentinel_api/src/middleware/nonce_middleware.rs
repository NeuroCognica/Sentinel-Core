...
use sentinel_core::{CanonicalEnvelopeAuthorizationRequest, NonceConsumed, IdentityEvent};
use sha2::Digest;
use hex;
...
pub fn check_and_append_nonce(
    ...
    // Check for any prior NonceConsumed for this actor+nonce (stored as Identity::NonceConsumed)
    for rec in events.iter() {
        if let EventKind::Identity = rec.kind {
            if let Ok(v) = serde_json::from_value::<IdentityEvent>(rec.payload.clone()) {
                if let IdentityEvent::NonceConsumed(typed) = v {
    ...
    // Wrap nonce in the IdentityEvent enum and mark the event kind as Identity
    let event = EventRecord {
        event_id: Uuid::new_v4(),
        timestamp_utc: now,
        actor: envelope.actor_id.to_string(),
        kind: EventKind::Identity,
        payload: serde_json::to_value(&IdentityEvent::NonceConsumed(nonce_typed))
            .map_err(|e| format!("payload ser: {e}"))?,
        prev_hash: None,
