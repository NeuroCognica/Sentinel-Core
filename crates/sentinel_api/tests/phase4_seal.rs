...
async fn test_phase4_effects_have_consent_and_evaluation() {
    ...
    let inner = json!({ "actor_id": "00000000-0000-0000-0000-000000000101", "key_id": "00000000-0000-0000-0000-000000000102", "public_key": pk });
    ...
    let inner = json!({ "issuer": "00000000-0000-0000-0000-000000000201", "subject": "00000000-0000-0000-0000-000000000202", "scope": "session", "actions": ["whoami"] });
    let req = test::TestRequest::post()
        .uri("/capabilities/issue")
        .set_json(&envelope_for("/capabilities/issue", inner))
        .to_request();
    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success());
    ...
    let req = test::TestRequest::post()
        ...
        .set_json(&envelope_for("/privileged/action", inner))
        .to_request();
    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success());
    ...
    for rec in events.iter() {
        match rec.kind {
            sentinel_store::EventKind::PolicyEvaluated => {
                if let Ok(pe) = serde_json::from_value::<sentinel_policy::event::PolicyEvaluated>(
                    rec.payload.clone(),
                ) {
            ...
            sentinel_store::EventKind::ConsentGranted => {
                if let Ok(ce) = serde_json::from_value::<sentinel_policy::event::ConsentEvent>(
                    rec.payload.clone(),
                ) {
            ...
            sentinel_store::EventKind::EffectExecuted => {
                ...
                if let Some(pd) = rec.payload.get("policy_digest").and_then(|v| v.as_str()) {
                    if let Some(id) = rec.payload.get("input_digest").and_then(|v| v.as_str()) {
                        ...
                        assert!(
                            policy_allows.contains(&key),
                            "EffectExecuted without prior PolicyEvaluated Allow: {:?}",
                            key
                        );
                        assert!(
                            consent_granted.contains(&key),
                            "EffectExecuted without prior ConsentGranted: {:?}",
                            key
                        );
                    }
                }
